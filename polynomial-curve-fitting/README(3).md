...
The generated application is a standalone HTML visualization that runs in a modern web browser.

> **Tip:** Add screenshots or a short GIF to `assets/` and place them here for a stronger GitHub landing page.

```text
assets/
├── demo.gif
├── interface.png
└── polynomial-degrees.png
```

For example:

```md
![Interactive polynomial curve fitting demonstration](assets/demo.gif)
```
...
Regenerates the noisy observations using the currently displayed settings and seed.
...
Optional screenshots and demonstration GIFs for the GitHub repository.
